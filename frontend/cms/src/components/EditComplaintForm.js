...
const EditComplaintForm = ({ complaint, closeForm }) => {
    ...
    try {
        ...
        const response = await axios.put(
            `http://localhost:5000/complaints/${complaint.fa_id}`,
            {
                assigned_to: selectedFitter, // will now be rep_cd string
                cms_status: "pending",
            }
        );
...
>
    ...
    {fitters.map((fitter) => (
        <option key={fitter.rep_cd || fitter.REP_CD} value={fitter.rep_cd || fitter.REP_CD}>
            {fitter.rep_cd || fitter.REP_CD}
        </option>
    ))}
</select>
